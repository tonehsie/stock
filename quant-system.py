...
# 設定網頁標題與佈局
st.set_page_config(page_title="台股全息量化系統 (V21.0 完美數據排版版)", layout="wide")
...
st.markdown("✅ **V21.0 專家雷達** | ✅ **智能千分位與小數點 (2位)** | ✅ **鉅額(3日)+神盾引擎**")
...
# 📌 1-2. V21.0 專家診斷引擎
# ==========================================
def get_expert_advice_v21(row, dead_chip_input, dynamic_dict, static_val):
    advice = []
...
def process_v21_ultimate_radar(df_wide, dead_chip_input, dynamic_dict, static_val, df_price):
    ...
    df['V21_實戰診斷'] = df.apply(lambda row: get_expert_advice_v21(row, dead_chip_input, dynamic_dict, static_val), axis=1)
    
    report_columns = ['日期', '收盤價(元)', '總人數變動率(%)', '1000張變動(%)', '作戰區變動(%)', 'K_Value', 'V21_實戰診斷']
...
# ==========================================
if run_btn:
    with st.spinner(f"正在擷取 {stock_id} 數據，並啟動 V21.0 完美排版版雷達..."):
        ...
        df_v21_radar = process_v21_ultimate_radar(df_share_wide, dead_chip_input, dynamic_dict, static_val, df_price)
        ...
        st.success("✅ V21.0 引擎運算完畢！數字皆已強制靠右，千分位與小數點完美呈現。")
        
        # 📌 智能排版與格式化引擎 (完全放棄危險的 Regex 取代，改用 Pandas Styler)
        ...
        show("▼▼▼ 1-2. V21.0 專家診斷雷達 (排版完美版) ▼▼▼", df_v21_radar, custom_class="radar-table")
        ...
        p += format_to_gas(df_v21_radar, "1-2. V21.0 專家診斷雷達 (完美排版版)")
